...
function getPeopleByLocation(people, location) {
  const matchingLocation = people.filter(
...
function translateToPirateTalk(phrase) {
  ...
  // Use map to translate each word if it exists in the lookup object
  const translatedWords = words.map((word) => {
    const pirateWord = EN_PIRATE_LOOKUP[word];
    return pirateWord || word; // If the word isn't found, use the original word
  });
...
function buildBugHuntCalendar(bugs) {
  ...
  // Create an empty array for each month (1 to 12)
  for (let month = 1; month <= 12; month++) {
    calendar[month] = [];
  }

  // Populate the calendar with bug names for each available month
